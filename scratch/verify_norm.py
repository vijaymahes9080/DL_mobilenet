...
def verify_emotion(emotion):
    ...
    # Test [0, 1]
    face_input_1 = (face_input_255 / 255.0)
    interpreter.set_tensor(input_details[0]['index'], face_input_1)
    interpreter.invoke()
    preds_1 = interpreter.get_tensor(output_details[0]['index'])[0]
    
    print(f"--- Emotion: {emotion} ---")
    print(f"[0, 255] Pred: {CLASS_NAMES[np.argmax(preds_255)]} ({np.max(preds_255):.2f})")
    print(f"[0, 1]   Pred: {CLASS_NAMES[np.argmax(preds_1)]} ({np.max(preds_1):.2f})")
