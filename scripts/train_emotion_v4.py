"""
ORIEN — Emotion CNN Trainer v4 (Final)
==========================================
Key Fixes vs v3:
  - Pre-load ALL data as numpy arrays (no on-the-fly tf.data augmentation ops)
  - Cap class weights at 3.0 max to prevent Disgust-weight explosion
  - Use standard numpy augmentation (fast flip, no crop)
  - Increase batch to 128 → fewer steps/epoch → much faster
  - No L2 on early layers → faster convergence from scratch

Target: 60-70% val_acc on FER2013 (SOTA ~73%)
"""
...
IMG_SIZE   = 48      # Native FER resolution (no upscale)
BATCH_SIZE = 128     # Big batch → fewer steps → fast CPU epoch
EPOCHS     = 80
...
def load_fer():
    ...
    print(f"  Class: { {CLASSES[k]: v for k,v in sorted(dist.items())} }")

    # Cap class weights at 3.0 to prevent gradient explosion on Disgust (only 436 samples)
    total = len(y_tr)
    n_cls = len(CLASSES)
    cw    = {k: min(3.0, total / (n_cls * v)) for k, v in dist.items()}
    print(f"  CW  : { {CLASSES[k]: f'{v:.2f}' for k,v in sorted(cw.items())} }")

    return X_tr, y_tr, X_va, y_va, cw

# ── Augmentation (numpy, fast) ────────────────────────────────────────────────
def augment_numpy(X: np.ndarray, y: np.ndarray) -> tuple:
    """Horizontal flip augmentation — creates 2× dataset size."""
    X_flipped = X[:, :, ::-1, :]      # flip width axis
...
def build_model(num_classes):
    ...
    x = layers.Conv2D(64, 3, padding='same', activation=act)(x)
    ...
    # Block 4 — 6×6 → 3×3
    x = layers.Conv2D(256, 3, padding='same', activation=act)(x)  # 256 to save RAM
    x = layers.BatchNormalization()(x)
    x = layers.MaxPooling2D(2)(x)
    x = layers.Dropout(0.40)(x)

    # Use GAP instead of Flatten to avoid huge gradient tensor
    x = layers.GlobalAveragePooling2D()(x)
    x = layers.Dense(512, activation=act)(x)
    x = layers.Dropout(0.50)(x)
    x = layers.Dense(256, activation=act)(x)
    x = layers.Dropout(0.40)(x)
    out = layers.Dense(num_classes, activation='softmax')(x)

    m = models.Model(inp, out, name="ORIEN_FER_v4")
    m.compile(
    ...
    print(f"  [BUILD] FER CNN v4 | params={params:,} ({params*4/1e6:.1f} MB)")
    return m
...
def main():
    ...
    print("\n" + "="*65)
    print("  ORIEN Emotion CNN v4  (FER2013 48×48 | Pre-augmented)")
    ...
    # Pre-augment training data (numpy flip — very fast, doubles samples)
    ...
    print(f"[INFO] {steps} steps/epoch @ batch={BATCH_SIZE}")
    ...
    print(f"\n[TRAIN] Launching {EPOCHS}-epoch run ...")
    history = model.fit(
        X_tr, y_tr,
